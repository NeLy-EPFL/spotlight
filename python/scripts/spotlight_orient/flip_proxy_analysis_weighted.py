...
should pull its score back up relative to a genuinely flipped frame's.
Writes to its own `OUTPUT_DIR`, separate from `flip_proxy_analysis.py`'s
own, so neither run overwrites the other's data.
...
Usage:
    python scripts/spotlight_orient/flip_proxy_analysis_weighted.py
"""
...
OUTPUT_DIR = (
    REPO_ROOT / "bulk_data/motion_prior/orient_model/flip_proxy_analysis_weighted"
)
N_BINS = 100
...
def load_trial_confidence(h5_path: Path) -> tuple[np.ndarray, np.ndarray, Path, int]:
    ...
    Returns:
        frame_idxs: Frame indices `confidence` corresponds to (a subset of
            `0..n_frames-1`).
        confidence: `weighted_confidence`, one value per `frame_idxs` entry.
    ...
    all_confidence = weighted_confidence(data["keypoint_scores"], data["node_names"])
...
def plot_pooled_histogram(
    ...
    ax.set_xlabel("Weighted confidence (proximal leg nodes x5, all others x1)")
