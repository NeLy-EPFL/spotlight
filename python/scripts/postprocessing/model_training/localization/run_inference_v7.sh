#!/bin/bash -l
# Runs v7's TinyLocalizationModel checkpoint over one full validation trial
...
#
# v7 is v6's exact hyperparameters, but with train.py's best-checkpoint/
# early-stopping criterion switched from val_loss to val_pixel_error
# directly (v6's own best-by-val_loss epoch wasn't its true best-by-
# pixel-error epoch), and PATIENCE raised 10->15. Ran the full 50 epochs
# without early stopping and landed on the SAME 19.32px best (epoch 42)
# as v6 -- since both share seed=0 and identical hyperparameters through
# that epoch, this is a deterministic confirmation, not a coincidence:
# 19.3px is a genuine plateau for this recipe, not an early-stopping
# artifact. More epochs alone don't help further; see run_training_v8.sh
# for the actual next change.
#
# TRIAL_STEM matches every earlier round's own choice, for a direct
# comparison on the same trial.
#
# Output filenames are suffixed "_v7", matching the other rounds' own
...
CHECKPOINT_PATH_REL="bulk_data/motion_prior/localization_model/checkpoints/v7/best.pt"
KEYPOINT_SET=neck
VIZ_CRF=23
MAX_VIZ_FRAMES=3000
...
predictions_h5="$output_dir/${TRIAL_STEM}_localization_predictions_v7.h5"
viz_path="$output_dir/${TRIAL_STEM}_localization_viz_v7.mp4"
