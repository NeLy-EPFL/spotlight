#!/usr/bin/env python
"""Runs a trained `TinyLocalizationModel` checkpoint over every cached frame of
one trial (see `scripts/spotlight_localization/cache_fullsize_frames.py`),
...
Usage:
    python scripts/spotlight_localization/infer.py \\
...
from spotlight_postprocessing.spotlight_localization.io_utils import (
...
def load_batch(paths: list[Path]) -> np.ndarray:
    ...
    single channel 3x to match `TinyLocalizationModel`'s RGB-shaped input.
    """
    images = []
    for path in paths:
...
def main(
    ...
    Args:
        ...
        frame_cache_root: Root directory
            `scripts/spotlight_localization/cache_fullsize_frames.py` wrote into.
        output_h5_path: Where to save predictions (see `io_utils.save_localization_predictions_h5`).
