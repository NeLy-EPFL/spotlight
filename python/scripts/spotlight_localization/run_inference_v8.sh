#!/bin/bash -l
# Runs v8's TinyLocalizationModel checkpoint over one full validation trial
...
#
# v8 is v7's exact setup but with TARGET_SIGMA_NATIVE_PX reduced 30->15
# (hypothesis: now that model.GlobalContextBlock fixed the receptive
# field, the model could commit to sharper heatmaps than a 30px target
# demanded). Result: heatmap_variance did drop (~0.005 -> ~0.003,
# confirming the heatmap genuinely got sharper), but val_pixel_error
# landed at 19.4px -- statistically the same as v6/v7's 19.32px
# plateau. Heatmap sharpness was NOT the bottleneck after all; see
# run_training_v9.sh (or later conversation) for what was tried next.
#
# TRIAL_STEM matches every earlier round's own choice, for a direct
# comparison on the same trial.
#
# Output filenames are suffixed "_v8", matching the other rounds' own
...
CHECKPOINT_PATH_REL="bulk_data/motion_prior/localization_model/checkpoints/v8/best.pt"
KEYPOINT_SET=neck
VIZ_CRF=23
MAX_VIZ_FRAMES=3000
...
predictions_h5="$output_dir/${TRIAL_STEM}_localization_predictions_v8.h5"
viz_path="$output_dir/${TRIAL_STEM}_localization_viz_v8.mp4"
