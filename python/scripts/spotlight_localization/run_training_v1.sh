#!/bin/bash -l
# First real training round for the TinyLocalizationModel, keypoint-only:
# coarse head/thorax/abdomen localization directly on the raw (unaligned,
# uncropped) camera frame at 0.25x resolution -- see
# src/poseforge/motion_prior/spotlight_localization/{model,dataset}.py.
#
...
# TinyLocalizationDataset loads into memory at once (see
...
checkpoint_dir="$repo_root/bulk_data/motion_prior/localization_model/checkpoints/v1"
