#!/bin/bash -l
# Runs v13's TinyLocalizationModel checkpoint over the same 4 trials used for
# v11/v12's own QA pass (predicting every cached frame, then rendering an
# annotated video for each) -- see run_training_v13.sh for what changed:
# v13 is a single from-scratch run (no warm-start, no frozen backbone),
# not a continuation of v11/v12's staged pipeline.
#
# Output filenames are suffixed "_v13", so re-running this doesn't clobber
# v11's/v12's own QA outputs for the same trials.
#
...
CHECKPOINT_PATH_REL="bulk_data/motion_prior/localization_model/checkpoints/v13/best.pt"
VIZ_CRF=23
MAX_VIZ_FRAMES=3000
...
for trial_stem in "${TRIAL_STEMS[@]}"; do
    predictions_h5="$output_dir/${trial_stem}_localization_predictions_v13.h5"
    viz_path="$output_dir/${trial_stem}_localization_viz_v13.mp4"
