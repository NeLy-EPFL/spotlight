...
fly's heading (yaw only) every frame, so the fly stays visually fixed on
screen while the grid rotates underneath it as it turns. cv2-only drawing
(no matplotlib); video I/O via `pvio`, GPU (NVENC) encoded.
...
from spotlight.postprocessing.pose2d.io_utils import load_pose_h5, load_skeleton_json
