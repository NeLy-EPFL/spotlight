...
The fly/world setup and DOF reorder below use the same tuned CPU-replay
values (actuator gain, adhesion, tarsal stiffness) validated against this
same QuickIK-solved body plan.
...
# the caller and passed in. Rebuilding the whole body plan per period
...
def replay_period_in_worker(
    ...
    **replay_period_kwargs,
    ...
    build; every call in one `replay/physics.py` invocation passes the same
