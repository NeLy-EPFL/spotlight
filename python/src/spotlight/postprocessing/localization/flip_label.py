...
`TinyLocalizationDataset` uses for its `flipped` target (True == flipped).
...
`scripts/postprocessing/model_training/localization/flip_proxy_analysis.py`, whose pooled histogram
...
from spotlight.postprocessing.localization.constants import FLIPPED_THRESHOLD
...
def proximal_leg_confidence(
    ...
    Args:
        keypoint_scores: `(n_frames, n_nodes)`, e.g. from
            `spotlight.postprocessing.pose2d.io_utils.load_pose_h5`.
