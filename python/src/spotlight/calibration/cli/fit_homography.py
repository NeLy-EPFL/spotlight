...
from spotlight.calibration.charuco import get_gizem_board


def load_muscle_frame(
    path: str, low_pct: float = 10, high_pct: float = 95
) -> np.ndarray:
    """Read 16-bit TIF muscle image and normalize to 0-255 using percentile clipping."""
    image = cv2.imread(str(path), cv2.IMREAD_UNCHANGED)
    q_low = np.percentile(image, low_pct)
    q_high = np.percentile(image, high_pct)
    image = (image - q_low) / (q_high - q_low)
    image = np.clip(image, 0, 1)
    return (image * 255).astype(np.uint8)


def load_behavior_frame(path: str) -> np.ndarray:
    """Read 8-bit JPG behavior image."""
...
def group_images_by_position(image_dir: Path, extension: str) -> dict:
    ...
    for img_path in image_paths:
        # Expected filename: homography_scan_x{x:.2f}_y{y:.2f}_frame{i}.ext
        # parts[2] = "x{x}", parts[3] = "y{y}"
...
def process_stage_position(
    ...
    behavior_frames = [load_behavior_frame(str(path)) for path in behavior_paths]
    muscle_frames = [
        load_muscle_frame(str(path), low_pct=10, high_pct=95) for path in muscle_paths
    ]
...
def fit_homography(
    profile_dir: Annotated[str, tyro.conf.arg(aliases=["-p"])] = "~/Spotlight/default/",
