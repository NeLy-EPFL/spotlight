...
# Tuned CPU-replay values (see module docstring), validated against this
# same QuickIK-solved body plan in both prior2d's and poseforge2's own
# FlyGym replay tools, and in poseforge's SeqIKPy-based one.
ACTUATOR_GAIN = 100.0  # uN*mm/rad
ADHESION_GAIN = 12.0  # uN
TARSAL_STIFFNESS = 8.0
...
ORBIT_CAM_AZIMUTH_OFFSET_DEG = 230.0
...
def build_fly_and_world(
    ...
    adhesion_gain: float = ADHESION_GAIN,
    tarsal_stiffness: float = TARSAL_STIFFNESS,
...
def replay_period(
    ...
    for step_idx in range(target_angles.shape[0]):
        ...
        while sim.time < target_time - half_physics_step:
            ...
            body_pos = sim.get_body_positions(fly_name)
...
def replay_period_in_worker(
    ...
    **replay_period_kwargs,
    ...
    here (once per worker process) rather than passed in.
    """
