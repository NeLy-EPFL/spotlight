...
solved IK joint angles (see `invkin.flygym_replay`), as a persisted pipeline
stage rather than a per-run throwaway file. `scripts/postprocessing/
replay_physics.py` writes this; `postprocessing.visualize`'s QA video reads
it back for the row-2 replay panel.

Schema
------
frames        (n_rendered, panel_size, panel_size, 3) uint8 RGB: one
              rendered frame per frame `replay_physics.py` actually attempted
