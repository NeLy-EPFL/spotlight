"""Shared I/O for `TinyOrientModel` predictions -- the dense per-frame `.h5`
schema `tools/spotlight_orient/infer.py` writes and
`tools/spotlight_orient/visualize_predictions.py` reads.
"""
...
def save_orient_predictions_h5(
    output_path: Path,
    ...
    """Save one trial's dense per-frame `TinyOrientModel` predictions.
...
def load_orient_predictions_h5(input_path: Path) -> dict:
    """Load an `.h5` file written by `save_orient_predictions_h5` back into
    a plain dict.
