...
`scripts/spotlight_localization/cache_fullsize_frames.py`), deriving labels from
...
from spotlight_postprocessing.spotlight_localization.augment import augment
...
def load_cached_frame(frame_path: Path) -> np.ndarray:
    ...
    if image is None:
        raise SystemExit(
            ...
            "scripts/spotlight_localization/cache_fullsize_frames.py first."
        )
    if image.ndim == 3:
        image = image[:, :, 0]
