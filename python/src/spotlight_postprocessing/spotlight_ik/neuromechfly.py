...
`../flygym/scripts/export_model_for_quickik.py`; see `scripts/spotlight_ik/
README_tools.md` for the exact command. Its 31 joints double as keypoints: the
...
# configuration in every sampled frame (not just occasionally); 0.01
# collapses into tangled, implausible poses outright -- the
# underconstrained-2D-observation failure mode QuickIK's own docs warn
# about. Set to 0.2 for production: tighter than 0.25, still on the stable
# side of the 0.1 cutoff.
NEUTRAL_WEIGHT = 0.2
