...
There are two types of commands: SET and LOG. The formats of these are as follows. Commands are communicated in compact JSON form and parsed/formatted using ArduinoJson.

## SET commands

```json
{
    "cmdTyp": "SET",  // fixed literal string
    "params": {
...
Upon a SET command, the triggering controller changes its state based on information in `params`.
...
- If `recording/opSequence` is empty, the recording is _open_: it keeps going on until the user stops it in the GUI, at which point the recorder sends the triggering controller a new SET command with `recording/isRecording = false` and new parameters for streaming specified in `params`.
- If `recording/opSequence` is not empty, the recording is _scheduled_: it executes steps in the `opSequence`. Each step shall be executed after the `frameIdx`-th behavior camera frame based on the following rules:
    ...
    - If `op` is STOP, the controller switches back to streaming mode. This entails reverting the parameters to the values immediately prior to the SET command initiating the recording. The channel is set to -1, as the operation is global. For technical reasons, `frameIdx` must be a multiple of 3 for STOP commands. Check this upon both serialization and parsing.

Upon SET command, the microcontroller should echo the following via the serial port: `Triggering controller received SET command`.
