# Spotlight documentation

## User's manual

- [Doing experiments](users_manual/doing_experiments.md)
- [Post-processing data](users_manual/postprocessing_data.md)
- [Quality-of-life improvements since previous version](users_manual/quality_of_life.md)
- [Troubleshooting](users_manual/troubleshooting.md)
- Optional: [Creating your custom arena](users_manual/custom_arena.md)
- Optional: [Frame rate limits](users_manual/frame_rate_limits.md)

## Developer's manual

- [Installation & compilation](developers_manual/installation_compilation.md)
- [Unit testing](developers_manual/unit_testing.md)
- [Data acquisition workflows](developers_manual/data_acquisition.md)
- [Code architecture](developers_manual/architecture.md)
- [Hardware](developers_manual/hardware.md)
- [Host-to-Arduino communication protocol](developers_manual/serial_comm_protocol.md)
- Spatial calibration
    - [Arena registration](developers_manual/arena_registration.md) for live tracking
    - [Camera homography](developers_manual/camera_homography.md) for behavior-to-muscle alignment
- [Code style](developers_manual/code_style.md)
- Instructions for AI agents live separately in [`AGENTS.md`](../AGENTS.md).