...
try `reset-camera muscle` (USB link reset, not a
true power cycle).
...
Status LED is solid green. `reset-camera muscle`
...
with `sudo`, which installs the udev rules needed for DFU upload.
