...
Arguments:
  ...
  -- ...           Everything after `--` is passed verbatim to `postprocess-recording`
                   (in addition to the always-set --recording-dir). If omitted, no extra
                   flags are passed.

Examples:
  spotlight_process_and_move_by_initials.sh VS XY -- --overwrite --with-muscle --use-homography
  spotlight_process_and_move_by_initials.sh VS XY -- --overwrite --no-with-muscle
EOF
}
...
for DIR in "${RECORDING_DIRS[@]}"; do
    ...
    if [[ ! -d "$PROCESSED_DIR" ]]; then
        echo "ERROR: processed folder not found in $DIR; keeping local data."
        FAILED+=("$DIR (no processed)")
        continue
    fi
    ...
    ####################################
    # Transfer + verify the processed/ folder
    ####################################
    echo "Transferring processed/ ..."
    if ! rsync -a --progress "$PROCESSED_DIR/" "$SERVER_DIR/processed/"; then
        echo "ERROR: rsync of processed/ failed."
        TRANSFER_OK=false
    elif ! verify_copy "$PROCESSED_DIR/" "$SERVER_DIR/processed/"; then
        echo "ERROR: verification of processed/ failed."
        TRANSFER_OK=false
    fi
