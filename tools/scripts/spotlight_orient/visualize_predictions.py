#!/usr/bin/env python
"""Renders a quick annotated video of one trial's `TinyOrientModel`
...
Usage:
    python tools/spotlight_orient/visualize_predictions.py \\
        --input-path bulk_data/.../orient_model/predictions/<trial>_orient_predictions.h5 \\
        ...
        --output-path <trial>_orient_predictions.mp4 \\
        --crf 23
"""
...
from spotlight_tools.spotlight_orient.io_utils import load_orient_predictions_h5
from spotlight_tools.spotlight_orient.model import TinyOrientModel, heatmap_probs
...
def predict_combined_heatmaps(
    ...
    """Each of `frame_indices`'s combined per-frame heatmap: `TinyOrientModel`'s
    ...
    Args:
        model: Trained `TinyOrientModel`, in eval mode.
        device: `"cuda"` or `"cpu"`.
...
def main(
    ...
    """Render an annotated orientation video for one trial.

    Args:
        input_path: `TinyOrientModel` predictions (see
            `infer.py`/`io_utils.save_orient_predictions_h5`).
        checkpoint_path: Trained `TinyOrientModel` state dict, re-run here
        ...
        frame_cache_root: Root directory
            `scripts/spotlight_orient/cache_fullsize_frames.py` wrote
    ...
    data = load_orient_predictions_h5(input_path)
