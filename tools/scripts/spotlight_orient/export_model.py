#!/usr/bin/env python
"""Exports a trained `TinyOrientModel` checkpoint to ONNX and TorchScript,
...
codebase needing this project's `TinyOrientModel` class at all (unlike a
...
Usage:
    python tools/spotlight_orient/export_model.py \\
...
from spotlight_tools.spotlight_orient.dataset import OUTPUT_SIZE
from spotlight_tools.spotlight_orient.model import TinyOrientModel
from spotlight_tools.common import export_onnx_and_torchscript
...
def main(
    ...
    """Export a trained `TinyOrientModel` checkpoint to ONNX and
    ...
    """
    model = TinyOrientModel(use_global_context=use_global_context)
