#!/bin/bash -l
# Fine-tunes iter1a further on a second round of manually-corrected labels
# (still no confidence-promoted pseudo-labels): warm-starts from
# iter1a/best.pt rather than iter0c/best.pt, continuing the same "warm
# start + corrected-labels-only" approach iter1a introduced. Run from the
# repo root.
#
# Rebuilds its own training data every time from CORRECTED_SLP, the same
# way run_training_iter1a.sh does (see its header comment for the pipeline
# and hyperparameter rationale, both unchanged here apart from warm-starting
# from iter1a's own checkpoint instead of iter0c's).
#
# Finishes with a QA pass: exhaustive (every frame) inference on one small
# trial with the resulting best.pt, then a quick annotated summary video
# (see visualize_predictions.py) -- a fast visual sanity check before
# trusting the checkpoint on the full dataset.
set -euo pipefail
...
DATA_DIR="bulk_data/motion_prior/2dpose_model/labels/rounds/iter2a"
...
checkpoint_dir="$repo_root/bulk_data/motion_prior/2dpose_model/checkpoints/iter2a"
