...
box directly on the raw fullsize frame, from `TinyOrientModel`'s own
neck/thorax/abdomen predictions (in raw pixel space) -- for QA
visualization (see `tools/spotlight_orient/visualize_predictions.py`)
...
aligned-domain predictions (`spotlight_tools.spotlight_pose2d`),
...
frames) from `TinyOrientModel`'s own predicted points vs. the canonical
...
from spotlight_tools.spotlight_orient.dataset import coarse_points_aligned_domain
from spotlight_tools.spotlight_pose2d.geometry import apply_affine, invert_affine
from spotlight_tools.spotlight_pose2d.io_utils import load_pose_h5
...
def raw_domain_box_corners(
    ...
    Returns:
        ...
        a NaN (e.g. a frame `TinyOrientModel` didn't predict).
    """
