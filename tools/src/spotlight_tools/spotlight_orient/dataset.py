"""Loads `TinyOrientModel` training examples from cached, pre-resized raw
(unaligned, uncropped) camera frames (see
`scripts/spotlight_orient/cache_fullsize_frames.py`), deriving labels from
an existing RepVGG-A0 model's exhaustive per-frame predictions (see
`spotlight_tools.spotlight_pose2d`) rather than any hand labels.
...
from spotlight_tools.spotlight_orient.augment import augment
from spotlight_tools.spotlight_orient.flip_label import (
    is_flipped,
    valid_frames,
    weighted_confidence,
)
from spotlight_tools.spotlight_pose2d.geometry import apply_affine, invert_affine
...
# Maps each coarse keypoint's output name to the RepVGG-A0 skeleton
# node(s) it's derived from -- one name for a direct node (e.g. "thorax"
# -> "Th"), several for a midpoint. Dict order is the model's own
# keypoint head output order.
...
SCALE_FACTOR = 4  # 0.25x, see model.TinyOrientModel's docstring.
OUTPUT_SIZE = (
...
# Same file/dataset `tools/spotlight_pose2d/slp_apply_alignment.py` reads.
...
def coarse_points_aligned_domain(
    ...
    `spotlight_pose2d.dataset.ALIGNED_FRAME_SIZE`).
    """
...
class TinyOrientDataset(Dataset):
