...
void setupFrameData(const std::string &shmFrameDataName, const size_t frameBufferSize,
    ...
    if (createNew) {
        ...
        shmFileDesc = shm_open(shmFrameDataName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (shmFileDesc == -1) {
            std::string errorMessage =
                "Failed to open shared memory for frame data: " + std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
        if (ftruncate(shmFileDesc, frameBufferSize) == -1) {
            std::string errorMessage = "Failed to set size of shared memory for frame data: " +
...
void setupShutterOpenTime(const std::string &shmShutterOpenTimeName,
    ...
    if (createNew) {
        shmFileDesc = shm_open(shmShutterOpenTimeName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (shmFileDesc == -1) {
            std::string errorMessage = "Failed to open shared memory for shutter-open time: " +
                                       std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
        if (ftruncate(shmFileDesc, shutterOpenTimeSize) == -1) {
            std::string errorMessage = "Failed to set size of shared memory for shutter-open "
                                       "time: " +
                                       std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
...
void setupFrameMetadata(const std::string &shmFrameMetadataName, FrameMetadata *&frameMetadataPtr,
    ...
    if (createNew) {
        shmFileDesc = shm_open(shmFrameMetadataName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (shmFileDesc == -1) {
            std::string errorMessage =
                "Failed to open shared memory for frame metadata: " + std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
        if (ftruncate(shmFileDesc, sizeof(FrameMetadata)) == -1) {
            std::string errorMessage = "Failed to set size of shared memory for frame metadata: " +
                                       std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
...
void setupMutex(const std::string &shmMutexName, pthread_mutex_t *&mutexPtr, bool createNew) {
    ...
    if (createNew) {
        shmFileDesc = shm_open(shmMutexName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (shmFileDesc == -1) {
            std::string errorMessage =
                "Failed to open shared memory for mutex: " + std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
        if (ftruncate(shmFileDesc, sizeof(pthread_mutex_t)) == -1) {
            std::string errorMessage =
                "Failed to set size of shared memory for mutex: " + std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
    } else {
...
void setupConditionVariable(const std::string &shmCondVarName, pthread_cond_t *&condVarPtr,
    ...
    if (createNew) {
        shmFileDesc = shm_open(shmCondVarName.c_str(), O_CREAT | O_RDWR | O_TRUNC, 0666);
        if (shmFileDesc == -1) {
            std::string errorMessage =
                "Failed to open shared memory for frame count: " + std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
        if (ftruncate(shmFileDesc, sizeof(pthread_cond_t)) == -1) {
            std::string errorMessage = "Failed to set size of shared memory for frame count: " +
                                       std::string(strerror(errno));
            spdlog::critical(errorMessage);
            throw std::runtime_error(errorMessage);
        }
    ...
    if (err != 0) {
        std::string msg = "Failed to initialize condition variable: " + std::string(strerror(err));
        spdlog::critical(msg);
