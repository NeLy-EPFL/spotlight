...
LinearMapper2x2to2::LinearMapper2x2to2()
    : w_X1toX(0), w_Y1toX(0), w_X2toX(0), w_Y2toX(0), biasX(0),
      w_X1toY(0), w_Y1toY(0), w_X2toY(0), w_Y2toY(0), biasY(0)
{
}
...
std::tuple<double, double> LinearMapper2x2to2::map(
...
CalibrationParams::CalibrationParams()
    : isDefined(false),
      stageAndPixelToPhysical(stageAndPixelToPhysical_),
    ...
    int pixelPosCol) const
{
    ...
    return stageAndPixelToPhysical_.map(stagePosX, stagePosY,
                                        pixelPosCol, pixelPosRow);
}
    ...
    double physicalPosY) const
{
    ...
    auto [col, row] = stageAndPhysicalToPixel_.map(stagePosX, stagePosY,
                                                   physicalPosX, physicalPosY);
    return {static_cast<int>(std::round(row)),
    ...
    int pixelPosCol) const
{
    ...
    return physicalAndPixelToStage_.map(physicalPosX, physicalPosY,
                                        pixelPosCol, pixelPosRow);
}
