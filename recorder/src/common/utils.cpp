...
cv::Mat makePseudoBGRImageFromThreeFrames(const GroupOfThreeFrames &groupOfThreeFrames) {
    std::vector<cv::Mat> channels = {groupOfThreeFrames.frame0.image,
...
std::string makeMetadataStringFromThreeFrames(const GroupOfThreeFrames &groupOfThreeFrames) {
    ...
    for (const FrameData &frameData :
        ...
        metadataString += fmt::format("{},{},{}\n", frameData.frameId, frameData.acquisitionTime,
...
std::string getSerialPortName(std::string deviceDescription, std::string deviceManufacturer) {
    ...
    spdlog::critical("Serial port not found. "
    ...
    for (SerialPortInfo serialPortInfo : allSerialPortInfo) {
        spdlog::critical("* Port name: '{}', description: '{}', manufacturer: '{}'",
...
fs::path prepareOutputFolder(const fs::path &directory, bool clearFolder) {
    ...
    if (!directory.empty() && directory.string().front() == '~') {
        ...
        if (homeDir) {
            ...
            spdlog::info("Expanded ~ in directory path '{}' to '{}'", directory.string(),
                         processedDir.string());
        } else {
    ...
    try {
        ...
        spdlog::error("Failed to create directory '{}' or clear its content: {}",
