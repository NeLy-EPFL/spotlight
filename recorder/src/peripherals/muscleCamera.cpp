...
MuscleCamera::MuscleCamera(unsigned int exposureTimeMicrosecs,
                           int imageWidth,
                           int imageHeight,
                           int xOffset,
                           int yOffset)
{
    ...
    spdlog::info("PCO camera default configuration retrieved");
    ...
    spdlog::info("Attempting to set PCO camera configuration");
...
FrameData MuscleCamera::waitForOneFrame()
{
    ...
    {
        spdlog::debug("Waiting for first frame from PCO camera");
        camera_.waitForFirstImage();
        firstFrameHasArrived_ = true;
    }
    else
    {
        spdlog::debug("Waiting for new image from PCO camera");
        camera_.waitForNewImage();
    }
    spdlog::debug("Image received from PCO camera");
...
/**
 * @brief Set exposure time in microseconds. Note that this can be called
